...
const initialState: ITrips = {
    open_sidebar: false,
    current_trips: [],
    planned_trips: localStorage.getItem("planned_trips")
...
const tripsSlice = createSlice({
    ...
    reducers: {
        ...
        delete_current_trip: (state, action) => {
            ...
            const deleted_index = state.current_trips.find((id) => id.country_name !== arg.country_name)
            state.current_trips = state.current_trips.filter((id) => id !== deleted_index)
        },
        ...
        save_planned_trip: (state, action) => {
            ...
            localStorage.setItem("planned_trips", JSON.stringify(state.planned_trips))
        },
        ...
        delete_planned_trips: (state, action) => {
            const arg = action.payload
            const deleted_index = state.planned_trips.find((id) => id.plan_name == arg.plan_name)
            state.planned_trips = state.planned_trips.filter((id) => id !== deleted_index)
        },
